...
public class PrefabBrush : GridBrushBase
{
    ...
    public override void Paint(GridLayout gridLayout, GameObject brushTarget, Vector3Int position)
    {
        ...
        if (itemInCell) { return; }

        if (position == previousPosition) { return; }
        previousPosition = position;

        if (brushTarget) { previousBrushTarget = brushTarget; }
    ...
    public override void Erase(GridLayout gridLayout, GameObject brushTarget, Vector3Int position)
    {
        ...
        if (ereased) { Undo.DestroyObjectImmediate(ereased.gameObject); }
    }
    ...
    private static Transform GetObjectInCell(GridLayout gridLayout, Transform parent, Vector3Int position)
    {
        ...
        // Checks child objects position agains bounds of tile.
